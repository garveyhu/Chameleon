...
async def dev_call_agent(
    *,
    ...
    if resume_answer is not None:
        # resume：从 pending 权威读 call_index + 原始 query（评审17 #3）。共享 helper，dev/playground/
        # embed 各 invoke 路径复用（engine/agent/durable）。
        from chameleon.engine.agent.durable import resolve_resume

        spec = await resolve_resume(target, run_id) if run_id else None
        ci = resume_call_index if resume_call_index is not None else (spec.call_index if spec else None)
        if ci is None:
        ...
        if spec and spec.query is not None:
            input = spec.query
    ctx = InvokeContext(
        agent_def=adef,
        input=input,
