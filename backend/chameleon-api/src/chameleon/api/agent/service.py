...
async def invoke(
    ...
    *,
    ...
    # durable HITL 续跑（与流式同契约，见 _resolve_resume_directive）
    resume_cvars, resume_rid, resume_query = await _resolve_resume_directive(
        req, agent_key, conv
    )
    ...
    # resume 不带 session 历史（对齐 embed / _prepare_invocation 语义）
...
async def _resolve_resume_directive(
    req: InvokeRequest, agent_key: str, conv: Any
) -> tuple[dict, str | None, str | None]:
    """durable HITL 续跑指令解析（流式 / 非流式共用）。

    校验 + 服务端权威读 pending（call_index / 原始 query / run_id）；scope 推导
    须与首跑一致：API 渠道 cvars 传 end_user_id，pending 落在
    `end_user_id or session_id`（agentkit_runner 的推导），与 embed/playground
    的 session scope 不同。客户端的 input 仅落历史，ctx 用原始 query 重放。

    Returns:
        (resume_cvars, resume_run_id, resume_query)；非 resume 请求返回空。
    """
    if req.resume_answer is None:
        return {}, None, None
    ...
    spec = await resolve_resume(agent_key, conv.end_user_id or conv.session_id)
    ...
    return (
        {"_resume_call_index": spec.call_index, "_resume_answer": req.resume_answer},
        spec.run_id,
        spec.query,
    )
...
async def _prepare_invocation(
    ...
    *,
    ...
    # resume 不带 session 历史（对齐 embed）：首跑上下文由 journal 重放重建，
    # history 又含首跑落库的原 query user msg——带上会让续跑后的新 LLM 调用
    # 看到重复 query（外加本次落库的人工回答）
    if resume_query is not None:
        history = []
