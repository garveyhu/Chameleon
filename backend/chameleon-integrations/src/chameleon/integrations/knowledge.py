...
async def search_kb(
    ...
    *,
    ...
    经 KbRetriever（LangChain BaseRetriever）调用 → on_retriever_start/end 原生回调自动
    落 retriever trace 节点（全路径覆盖），本函数无 trace 代码。
    """
    from chameleon.integrations.retrievers import get_kb_retriever

    retriever = get_kb_retriever(kb_key, top_k=top_k, min_score=min_score)
