...
# 外部包通过该 entry-point group 注册 agent（命名空间外也能被发现）：
#   [project.entry-points."chameleon.agents"]
#   my-agent = "my_pkg.my_module"        # 指向模块 → 扫其中 @agent / BaseAgent
#   my-agent = "my_pkg.mod:handle"       # 直接指向 @agent 函数 / BaseAgent 类
_AGENT_ENTRY_POINT_GROUP = "chameleon.agents"
...
def _index_module(
    mod: Any, base_index: dict[str, type], agentkit_index: dict[str, Any]
) -> None:
    """从一个模块收 BaseAgent 子类（老范式）+ @agent 目标（agentkit）写入索引。"""
...
def _index_target(
    obj: Any, base_index: dict[str, type], agentkit_index: dict[str, Any]
) -> None:
    """收一个 entry-point 直接指向的对象（@agent 函数 / BaseAgent 类）。"""
    manifest = getattr(obj, "__agent_manifest__", None)
    if manifest is not None:
        agentkit_index[manifest.key] = obj
        return
    from chameleon.core.base.agent_router import agent_router
    from chameleon.core.base.base_agent import BaseAgent
    ...
    if isinstance(obj, type) and issubclass(obj, BaseAgent) and obj is not BaseAgent:
        ...
        if meta.id:
            base_index[meta.id] = obj
            try:
                agent_router.register(obj)
            except Exception as e:
                logger.warning("agent_router.register failed for {}: {}", meta.id, e)
...
def _scan_namespace_agents(
    ...
    try:
        ...
        logger.warning("chameleon.agents namespace not found")
        return
    for mod_info in pkgutil.iter_modules(pkg.__path__, "chameleon.agents."):
        ...
        _index_module(mod, base_index, agentkit_index)


def _scan_entry_point_agents(
    base_index: dict[str, type], agentkit_index: dict[str, Any]
) -> None:
    """扫 entry-point group=chameleon.agents —— 命名空间外的外部包也能注册。"""
    try:
        eps = importlib.metadata.entry_points(group=_AGENT_ENTRY_POINT_GROUP)
    except Exception as e:  # noqa: BLE001
        logger.warning("agent entry-point 扫描失败：{}", e)
        return
    found: list[str] = []
    for ep in eps:
        try:
            ...
            raise RegistryError(
                message=f"failed to load agent entry point {ep.name}: {e}"
            ) from e
        if inspect.ismodule(obj):
            _index_module(obj, base_index, agentkit_index)
        else:
            _index_target(obj, base_index, agentkit_index)
        found.append(ep.name)
    if found:
        logger.info("entry-point agents discovered: {}", found)


def _scan_local_agent_modules() -> tuple[dict[str, type], dict[str, Any]]:
    """发现本地 agent：命名空间扫描 + entry-point（外部包）两路合并去重。
    ...
    _scan_namespace_agents(base_index, agentkit_index)
    _scan_entry_point_agents(base_index, agentkit_index)
