...
## 13. 定位决定（2026-06-09，用户拍板）：内部 SDK

用户明确选择 **"只当内部 SDK"**（非对外开源发布）。据此：
- **不落 LICENSE、不发 PyPI**——`chameleon-agentkit` 是 Chameleon 平台内置的 agent 开发库，面向本
  仓库/平台内的 agent 作者，不对外公开分发。
- README / guide / cookbook / 包内 docstring 的"pip install chameleon-agentkit（公开包）/ 冻结公共
  API / 对外采用"话术已改为**内部 SDK 定位**（workspace 可编辑装；导出面=站内 agent 依赖的**内部
  稳定契约**，冻结门禁仍保留作内部稳定性保障）。
- **"世界顶级开源"锚撤下**：本框架的目标态是 **~8.3 的优秀内部 agent 编排 SDK**（功能/质量/安全已
  达此水准），不再以"外部采用规模"为衡量项。"外部采用"杠杆按用户决定**主动放弃**，非未竟缺口。
- standalone/脱平台模式保留为**作者本机自测**手段（提交进站前快速验证），非外部分发产品。

**结论：按内部 SDK 定位，框架已达成目标态。** 后续仅按真实内部需求/生产信号增量演进（如真要多租户
对外 A2A 再补 api_key scope），不为"对外发布"做投机工作。
